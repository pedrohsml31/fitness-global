...
class HealthPlugin : Plugin() {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                for (workout in response.records) {
                    ...
                    // extras (Fitness Global): tipo cru + como o registro foi criado
                    workoutObject.put("exerciseTypeRaw", workout.exerciseType)
                    workoutObject.put("recordingMethod", workout.metadata.recordingMethod)
                    workoutObject.put("deviceModel", workout.metadata.device?.model ?: "")
                    workoutObject.put("deviceType", workout.metadata.device?.type ?: -1)
    ...
    // Fitness Global: lê os registros de peso (ex.: balança Yunmai -> Samsung Health -> Health Connect)
    @PluginMethod
    fun queryWeight(call: PluginCall) {
        ...
        val request = ReadRecordsRequest(WeightRecord::class, timeRange, emptySet(), true, 1000)
